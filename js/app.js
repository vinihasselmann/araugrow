...
cardapio.metodos = {
    ...
    buscarPorNome: (nome) => {
        ...
        // Iterate through each category in MENU
        for (const categoria in MENU) {
            if (MENU.hasOwnProperty(categoria)) {
                // Filter items within the category by name
                let itensFiltrados = MENU[categoria].filter(item => 
                    item.name && item.name.includes(nome)
                );
                
                // Add the filtered items to the resultados array
                resultados = resultados.concat(itensFiltrados.map(item => {
                    return {
                        categoria: categoria,
                        item: item
                    };
        ...
        resultados.forEach(result => {
            let temp = cardapio.templates.item
                .replace(/\${img}/g, result.item.img)
                .replace(/\${name}/g, result.item.name)
                .replace(/\${dsc}/g, result.item.dsc)
                .replace(/\${price}/g, result.item.price.toFixed(2).replace(".", ","))
                .replace(/\${id}/g, result.item.id);
    
            $("#itensCardapio").append(temp);
        });
    
        // Update the active category to indicate search results
        $(".container-menu a").removeClass('active');
        $("#menu-todos").addClass('active');
    },
    
    obterItensCardapio: (categoria = "sedas", vermais = false) => {
    ...
    resumoPedido: () => {
        ...
        let observacoes = $("#txtObservacoes").val().trim();
    ...
    calcularEntrega: (callback) => {
        if (MEU_ENDERECO !== null) {
            const service = new google.maps.DistanceMatrixService();
            const request = {
                origins: ["Helena Piekarski Pinto, 456, Fazenda Velha, Araucária, PR, 83704650"],
                destinations: [`${MEU_ENDERECO.endereco}, ${MEU_ENDERECO.numero}, ${MEU_ENDERECO.bairro}, ${MEU_ENDERECO.cidade}, ${MEU_ENDERECO.uf}, ${MEU_ENDERECO.cep}`],
            ...
            service.getDistanceMatrix(request, (response, status) => {
                ...
                const numericDistance = parseFloat(distancia.replace(/[^\d.]/g, ''));
                const distanciaEmKm = (numericDistance / 10).toFixed(1);
                
                // Update VALOR_ENTREGA
                VALOR_ENTREGA = cardapio.metodos.calcularValorEntrega(distanciaEmKm);
    
                // Call the callback with the updated VALOR_ENTREGA
                if (callback && typeof callback === 'function') {
                    callback(VALOR_ENTREGA);
                    cardapio.metodos.carregarValores();
                }
            });
        }
    ...
    calcularValorEntrega: (DISTANCIA) => {

        const costPerKm = 1.3;

                const deliveryCost = DISTANCIA * costPerKm;
    ...
    finalizarPedido: () => {
        if (MEU_CARRINHO.length > 0 && MEU_ENDERECO !== null) {
            ...
            // Ensure VALOR_ENTREGA is updated by passing a callback
            cardapio.metodos.calcularEntrega((updatedValorEntrega) => {
                VALOR_ENTREGA = updatedValorEntrega;
    
                $.each(MEU_CARRINHO, (i, e) => {
                    itens += `*${e.qntd}x* ${e.name} ...... R$ ${e.price.toFixed(2).replace('.',',')} ${e.obs}\n`;
    ...
    itemCarrinho:`
                    <div class="col-12 item-carrinho">
                        ...
                        <div class="dados-produto">
                            ...
                            <div class ="wrapper-produtos-carrinho">
                                <p class="descricao-produto-carrinho text-center mt-4">
                                    <b>\${dsc}</b>
                                </p>
                            

                            <div class="form-group ml-4 mr-4 form-observacoes-carrinho">
                                <label>Observações:</label>
                                <input id="txtObservacoes" type="text" class="form-control"/>
                            </div>
                            </div>
